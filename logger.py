...
class Logger(object):
    def __init__(self, k_fold=None, num_classes=None, params=None, args=None):
        super().__init__()
        self.k_fold = k_fold
        self.num_classes = num_classes
        self.initialize(k=None)
        self.params = params
        self.csv_name = f'metric.csv'
    ...
    def evaluate(self, k=None, initialize=False, option='mean'):
        ...
        if not self.k_fold is None and k is None:
            ...
            precision = aggregate([metrics.precision_score(samples['true'][k], samples['pred'][k], average='macro') for k in range(self.k_fold)])
            recall = aggregate([metrics.recall_score(samples['true'][k], samples['pred'][k], average='macro') for k in range(self.k_fold)])
            ...
            precision = metrics.precision_score(samples['true'], samples['pred'], average='macro')
            recall = metrics.recall_score(samples['true'], samples['pred'], average='macro')
    ...
    def to_csv(self, targetdir, k=None, initialize=False):
        ...
        append = os.path.isfile(os.path.join(targetdir, self.csv_name))
        with open(os.path.join(targetdir, self.csv_name), 'a', newline='') as f:
            writer = csv.writer(f)
            if not append:
